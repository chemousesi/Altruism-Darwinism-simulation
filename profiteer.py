...
class Profiteer(Agent):

    def get_color():
        return RED

    def find_closest_pheromone(self, list_of_pheromones):
        # finds the closest pheromon works for profiteers and altruists 
            ...
            if food.ressource > 0:
                for food_box in food.table: #we could check if the agent is in the food before checking every single food_box in the food
                    if food_box == self.pos:
                        ...
                        # i deleted that responsible part of the code that generates pheromones
